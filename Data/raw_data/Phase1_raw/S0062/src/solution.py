...
def suggest_slots(events: List[Dict[str, str]], meeting_duration: int, day: str) -> List[str]:
    ...
    # Adjust WORK_END for Friday soft cutoff (if needed)
    weekday = datetime.strptime(day, "%Y-%m-%d").weekday()  # Monday=0 ... Friday=4
    if weekday == 4:  # Friday
        # Latest meeting must end by 17:00, but start by 15:00 for 30-min meeting
        WORK_END = 17*60
        WORK_START = 9*60
    ...
    for e in events:
        ...
        if start < end:
            busy_intervals.append([start, end + BUFFER_AFTER_EVENT])  # buffer after events
    ...
    while candidate_start + meeting_duration <= WORK_END:
        ...
        for b_start, b_end in busy_intervals:
            if candidate_start < b_end and candidate_end > b_start:
                overlap = True
                # Skip candidate_start directly to end of this busy interval
