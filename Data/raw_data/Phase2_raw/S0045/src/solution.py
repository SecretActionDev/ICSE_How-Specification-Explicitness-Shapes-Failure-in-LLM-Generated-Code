...
def suggest_slots(events: List[Dict[str, str]], meeting_duration: int, day: str) -> List[str]:
    ...
    if date_obj.weekday() == 4:  # Friday
        WORK_END = min(WORK_END, time_to_minutes("15:00"))
    
    # Convert events to minutes and add lunch
    blocked = [(max(WORK_START, time_to_minutes(e["start"])), 
                min(WORK_END, time_to_minutes(e["end"]))) 
               for e in events]
    ...
    while current_time + meeting_duration <= WORK_END:
        overlap = any(not (current_time + meeting_duration <= b_start or current_time >= b_end) 
