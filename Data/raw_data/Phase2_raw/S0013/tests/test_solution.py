...
#Description: If a requested resource exactly equals the available quantity for that resource, the allocation should be considered NOT feasible (False). 
...
# New Test Case 1: Empty resource dictionary and empty request list (No resources, no requests)
def test_empty_resource():
    resources = {}
    requests = [{'cpu': 5, 'ram': 5}]
    assert is_allocation_feasible(resources, requests) is False  # No resources available, should return False

# New Test Case 2: Empty resource dictionary and empty request list (No resources, no requests)
def test_empty_resource_and_empty_requests():
    resources = {}
    requests = []
    assert is_allocation_feasible(resources, requests) is False  # No resources and no requests, should return False


# New Test Case 3: Negative resources and empty request list
def test_empty_resource_and_empty_requests():
    resources = {"cpu": -5}
    requests = []
    assert is_allocation_feasible(resources, requests) is False