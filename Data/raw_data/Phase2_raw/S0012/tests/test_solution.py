...
# -------------------------------------------------
# Part B – Friday Restriction Tests
# -------------------------------------------------

def test_friday_excludes_start_times_after_1500():
    """
    Functional requirement:
    On Fridays, no meeting may start after 15:00.
    """
    events = []
...
def test_friday_allows_start_time_exactly_1500():
    """
    Boundary condition:
    15:00 should still be allowed on Friday.
    """
    events = []
    slots = suggest_slots(events, meeting_duration=30, day="2026-02-06")  # Friday
...
def test_non_friday_not_affected_by_cutoff():
    """
    Functional requirement:
    The 15:00 restriction applies only to Fridays.
    """
    events = []
...
def test_friday_with_existing_event_before_cutoff():
    """
    Edge case:
    Event before 15:00 should not interfere with Friday restriction logic.
    """
    events = [{"start": "14:00", "end": "14:30"}]
    slots = suggest_slots(events, meeting_duration=30, day="2026-02-06")  # Friday

    assert "14:00" not in slots
    assert "15:15" not in slots  # Still blocked due to Friday rule


def test_friday_long_meeting_crossing_cutoff():
    """
    Edge case:
    Meeting starting before 15:00 but ending after 15:00 is allowed,
    since restriction applies only to start time.
    """
    events = []
    slots = suggest_slots(events, meeting_duration=120, day="2026-02-06")  # Friday

    assert "14:00" in slots  # 14:00–16:00 allowed
    assert "15:15" not in slots